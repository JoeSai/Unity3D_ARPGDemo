...
namespace YM.Dungen {
    public class DungenGenerator : MonoBehaviour
    {
        ...
        public float m_voxelSize = 6;//体素的尺寸
        ...
        RandomInt m_random;
        public bool m_isFinishGen = false;
        ...
        void Start()
        {
            ...
            GenTest = GenDungen();
            StartCoroutine(GenTest);
            
            StartPos = GameObject.Find("StartLocation").transform;
            Transform player1 = Instantiate(player, StartPos.position, Quaternion.identity);
            player1.parent = StartPos;
            FollowPlayer Fp = Camer.AddComponent<FollowPlayer>() as FollowPlayer;
        }
        ...
        private bool CheckGenerateMap()
        {
            ...
            Debug.Log(battleRoom_S+"房间1数");
            Debug.Log(battleRoom_B + "房间2数");

            if (battleRoom_S < 2 || battleRoom_B < 1)
            {             
                return false;
            }
            else
        ...
        private void Regenerate()
        {
            ...
            m_doors.Clear();
            m_openSet.Clear();
            m_globalVoxels.Clear();
            m_connectorsPos.Clear();
